...
class SellHandler(BaseDbHandler):
    ...
    async def delete(self, name: str, rowStr: str, colStr: str):
        ...
        if user.name != name:
            self.logWarn(f"Got sell request for {name} from {user.name}.")
            self.set_status(403) # Forbidden
            return

        # Check that the row and column are within the playfield
        if row < 0 or row >= self.gameConfig.playfield.numRows:
            self.logWarn(f"Got invalid sell request for row {row} of {self.gameConfig.playfield.numRows}.")
            self.set_status(404) # Not found
            return
        if col < 0 or col >= self.gameConfig.playfield.numCols:
            self.logWarn(f"Got invalid sell request for col {col} of {self.gameConfig.playfield.numCols}.")
            self.set_status(404) # Not found
            return
